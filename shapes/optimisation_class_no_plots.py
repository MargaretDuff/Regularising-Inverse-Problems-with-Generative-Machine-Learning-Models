...
from tv_functional import TV
from tv_algorithm import PALM as tv_palm
...
class optimisation():
    ...
    def gd_z_regularisation_parameter(self, mask=None, psnr=False, alpha_min=0.0001, alpha_max=100, alpha_factor=0.5, initial_z=None, iteration_number=100, save_name='None', save=False, early_stop=False):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        if early_stop:
        ...
        while alpha>alpha_min:
            ...
            if psnr:
                ...
                if mask is None:
                    ...
                    loss=np.linalg.norm((self.G(z)-self.aim)[mask==1])**2
    ...
    def optim_z_sparse_deviations(self,alpha=0.1, beta=0.8, initial_z=None, initial_u=None, iteration_number=50 ):
        ...
        f1= odl.solvers.L2NormSquared(self.y_space).translated(self.data)
        f=odl.solvers.FunctionalComp(f1,AGu)
        ...
        return(proj_z(final_result), proj_x(final_result))
         ...
         # ||A(G(z)+u)-y||_2^2+\lambda\mu||z||^2_2+\lamba||u||_1
    def optim_z_sparse_regularisation_parameter(self, mask=None,  psnr=False,lambda_min=0.01, lambda_max=100, lambda_factor=0.5, mu_min=0.01, mu_max=2, mu_factor=0.5,initial_z=None, initial_u=None, iteration_number=100, save_name='None', early_stop=False, save=False):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        if initial_u is None:
            initial_u = np.zeros((1, *self.generative_model.image_shape))
            ...
            while lamb>lambda_min:
                ...
                if psnr:
                    ...
                    if mask is None:
                        ...
                        loss=np.linalg.norm((self.G(z)+u-self.aim)[mask==1])**2
                if save:
    ...
    def optim_z_sparse_tv_deviations(self,alpha=0.1, beta=0.8, initial_z=None, initial_u=None, iteration_number=50 ):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        if initial_u is None:
            initial_u = np.random.normal(0,1,(1, *self.generative_model.image_shape))
        if len(initial_u.shape)==2:
            initial_u=np.expand_dims(initial_u, axis=0)
        ...
        for i in range(initial_z.shape[0]):
            ...
            callback=odl.solvers.util.callback.CallbackStore(results=results)*(f+g1*proj_z+g2*proj_x)
            zu_initial=zu_space.element((np.expand_dims(initial_z[i], axis=0), initial_u[i]))
            print('BEGINNING IMPORTANT HERE.....',odl.solvers.GroupL1Norm(grad.range)(grad(proj_x(zu_initial))))
            optimised=tv_palm(f,g, callback=callback,niter=iteration_number, x=zu_initial )
            if f(optimised.x)+g1(proj_z(optimised.x))+g2(proj_x(optimised.x))<f(final_result)+g1(proj_z(final_result))+g2(proj_x(final_result)):
                final_result=optimised.x
        print('IMPORTANT HERE.....', g2(proj_x(optimised.x)))
        return(proj_z(final_result), proj_x(final_result))

         # ||A(G(z)+u)-y||_2^2+\lambda\mu||z||^2_2+\lamba||u||_1
    def optim_z_sparse_tv_regularisation_parameter(self, mask=None,  psnr=False,lambda_min=0.01, lambda_max=100, lambda_factor=0.5, mu_min=0.01, mu_max=2, mu_factor=0.5,initial_z=None, initial_u=None, iteration_number=100, save_name='None'\
, early_stop=False, save=False):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        if initial_u is None:
            initial_u = np.zeros((1, *self.generative_model.image_shape))
        ...
        while mu>mu_min:
            ...
            inner_count=0
            while lamb>lambda_min:
                z,u=self.optim_z_sparse_tv_deviations( lamb*mu,lamb, initial_z, initial_u, iteration_number)
                if psnr:
                    loss=skimage.measure.compare_psnr( self.aim,self.G(z)+u,2)
                else:
                    if mask is None:
                ...
                if save:
                    ...
                    np.save(save_name+'lambda_'+str(lamb)+'_mu_'+str(mu)+'.npy',self.G( z)+u)
                    np.save(save_name+'lambda_'+str(lamb)+'_mu_'+str(mu)+'_in_range.npy',self.G( z))
                    np.save(save_name+'lambda_'+str(lamb)+'_mu_'+str(mu)+'_sparse_addition.npy',u)
                plot[count,:]=[lamb, mu, loss]
                count+=1
                inner_count+=1
                if abs(loss)< stop:
    ...
    def optim_x_soft_constraints_regularisation_parameter(self, mask=None,psnr=False, lambda_min=0.01, lambda_max=100, lambda_factor=0.25, mu_min=0.01, mu_max=10, mu_factor=0.25,initial_z=None, initial_x=None, iteration_number=100, save_name='None', early_stop=False, save=False):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        if initial_x is None:
            initial_x = np.random.normal(0,1,(1,*self.generative_model.image_shape))  
            ...
            while lamb>lambda_min:
                ...
                if psnr:
                    ...
                    if mask is None:
                        ...
                        loss=np.linalg.norm((x-self.aim)[mask==1])**2
    ...
    def optim_x_tik_regularisation_parameter(self, mask=None, psnr=False, beta_min=0.0001, beta_max=100, beta_factor=0.5, initial_x=None, iteration_number=100, save_name='None', save=False, early_stop=False):
        if initial_x is None:
            initial_x = np.random.normal(0,1,(1,*self.generative_model.image_shape))
        if len(initial_x.shape)==2:
            initial_x=np.expand_dims(initial_x, axis=0)
    ...
    def optim_x_tv(self, beta, initial_x=None, iteration_number=100):
        ...
        op_norm = 1.1 * odl.power_method_opnorm(L, maxiter=4, xstart=test_images[0])
    ...
    def optim_x_tv_regularisation_parameter(self,mask=None, psnr=False, beta_min=0.0001, beta_max=100, beta_factor=0.5, initial_x=None, iteration_number=100, save_name='None', save=False, early_stop=False):
        if initial_x is None:
            initial_x = np.random.normal(0,1,(1,*self.generative_model.image_shape))
        if len(initial_x.shape)==2:
            initial_x=np.expand_dims(initial_x, axis=0)
