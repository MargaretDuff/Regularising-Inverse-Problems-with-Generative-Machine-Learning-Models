...
class invProb():
    def __init__(self,  generative_model, name='denoising',   kernel=(1/273)*np.array([[1,4,7,4,1],[4,16,26,16,4],[7,26,41,26,7],[4,16,26,16,4],[1,4,7,4,1]]),dim_comp=140, angles=30, inpainting_frac=0.25):
        self.name=name
        ...
        ##For denosiing
            ...
            flatten=odl.operator.tensor_ops.FlatteningOperator(self.x_space)  
            cs=odl.operator.tensor_ops.MatrixOperator(matrix, domain=flatten.range)
            A_part=odl.operator.operator.OperatorComp(cs, flatten)
