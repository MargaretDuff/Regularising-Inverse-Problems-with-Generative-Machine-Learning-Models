...
batch_size = 64
try:
    opts, args = getopt.getopt(sys.argv[1:], "hs:n:a:b:r:", [
                               "save=", "n_latent=", "alpha=", "beta=", "rho="])
...
# Set up the loading and saving of checkpoints
saver = tf.train.Saver()
if beta == 0:
    save_file = './checkpoints_ns/checkpoints' + \
        str(n_latent)+'/mnist_AE_'+str(n_latent)
    if save:
        sess.run(tf.global_variables_initializer())
        saver.save(sess, save_file, global_step=0, write_meta_graph=True)
    else:
        saver.restore(sess, './checkpoints'+str(n_latent)+'_' +
                      str(alpha)+'/mnist_AE_'+str(n_latent)+'_'+str(alpha)+'-29800')
else:
    save_file = './checkpoints_' + \
        str(n_latent)+'_'+str(beta)+'_'+str(rho) + \
        '/mnist_AE_'+str(n_latent)+'_'+str(alpha)
    if save:
        sess.run(tf.global_variables_initializer())
        saver.save(sess, save_file, global_step=0, write_meta_graph=True)
    else:
        saver.restore(sess, './checkpoints_'+str(n_latent)+'_'+str(beta) +
                      '_'+str(rho)+'/mnist_AE_'+str(n_latent)+'_'+str(alpha)+'-29800')
# Training loop
if save:
    for i in range(30000):
        ...
        if not i % 200:
            ...
            ls, d, i_ls, d_ls, mu = sess.run([loss, dec, img_loss, sparse_loss, sampled], feed_dict={
