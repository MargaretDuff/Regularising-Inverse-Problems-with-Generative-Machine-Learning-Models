...
class generative_model(object):
        ...
        for j in range(initial_z.shape[0]):
            ...
            for i in range(100000):
                ...
                if np.all(self.observationLoss(z_1,y)<= \
                              ...
                              +(1/(2*100*L))*np.linalg.norm(self.gradients(z_0,y), ord=2)**2):
                    z_0=z_1
                    count+=1
                    L=lower*L
                else:
    ...
    def projectManifold(self,x, saveName='test', restore=True, noise_level=0.2, random_seed=99):
        print(np.shape(x))
        ...
        if not restore:
            ...
            import glob
            import os
            filesEncodings= glob.glob(saveName+'CheckpointNoisyGEncodings*.npy')
            filesLoss=glob.glob(saveName+'CheckpointNoisyGLoss*.npy')
            filesNoisy=glob.glob(saveName+'CheckpointNoisyGImages*.npy')
            filesProject=glob.glob(saveName+'CheckpointProjectGImages*.npy')


            try:
                filesEncodings.sort(key=os.path.getmtime)
                filesLoss.sort(key=os.path.getmtime)
                filesNoisy.sort(key=os.path.getmtime)
                filesProject.sort(key=os.path.getmtime)
                encodings=np.load(filesEncodings[-1])
                loss=np.load(filesLoss[-1])
                noisy=np.load(filesNoisy[-1])
                project=np.load(filesProject[-1])
                start=int(filesEncodings[-1][len(saveName+'CheckpointNoisyGEncodings'):-4])
                print('Restored encodings at number' +str(start))
            except:
                loss=np.zeros((number,2))
                encodings=np.zeros((number, self.n_latent))
                noisy=np.zeros((number, *self.image_shape))
                project=np.zeros((number, *self.image_shape))
                start=0

        x_noise=x+noise_level*np.random.normal(0,1, x.shape) #Add noise here!!
        for i in range(start,number):
            encodings[i,:]=self.encode(x_noise[i,:,:])
            loss[i,0]=self.observationLoss(encodings[i,:], x[i,:,:])
            loss[i,1]=self.observationLoss(encodings[i,:], x_noise[i,:,:])
            noisy[i]=x_noise[i]
            project[i]=self.generate(encodings[i,:])
            if i%50==0:
                np.save(saveName+'CheckpointNoisyGEncodings'+str(i)+'.npy', encodings)
                try:
                    os.remove(saveName+'CheckpointNoisyGEncodings'+str(i-50)+'.npy')
                except:	
                    pass
                np.save(saveName+'CheckpointNoisyGLoss'+str(i)+'.npy', loss)
                try:
                    os.remove(saveName+'CheckpointNoisyGLoss'+str(i-50)+'.npy')
                except:	
                    pass
                np.save(saveName+'CheckpointNoisyGImages'+str(i)+'.npy', noisy)
                try:
                    os.remove(saveName+'CheckpointNoisyGImages'+str(i-50)+'.npy')
                except:
                    pass
                np.save(saveName+'CheckpointProjectGImages'+str(i)+'.npy', project)
                try:
                    os.remove(saveName+'CheckpointProjectGImages'+str(i-50)+'.npy')
                except:
                    pass
        np.save(saveName+'NoisyGImages.npy', noisy)
        np.save(saveName+'NoisyGProjectImages.npy',project)
        np.save(saveName+'NoisyGEncodings.npy', encodings)
        np.save(saveName+'NoisyGInOutLoss.npy',loss)
        return(encodings,loss)
