...
try:
    opts, args = getopt.getopt(sys.argv[1:], "hs:n:r:b:e:k:", [
...
#Load training images and define new batch function 
...
#Set up the checkpoint saver and loader 
saver = tf.train.Saver()
save_file = './checkpoints_no_sigmoid_' + \
    str(n_latent)+'_kl_factor_'+str(kl_factor)+'/checkpoints'+str(n_latent)
...
def generate_images(epoch):
    ...
    test_image_stack = (test_image_stack -
                        np.min(test_image_stack))*255/np.max(test_image_stack)
    test_image_stack = np.squeeze(np.round(test_image_stack).astype(np.uint8))
    tiled_output = tile_images(test_image_stack)
    tiled_output = Image.fromarray(
        tiled_output, mode='L')  # L specifies greyscale
    outfile = os.path.join('./checkpoints_no_sigmoid_'+str(n_latent) +
                           '_kl_factor_'+str(kl_factor), 'epoch_{}.png'.format(epoch))
