...
class optimisation():
    ...
    def gd_z_regularisation_parameter(self, alpha_min=0.01, alpha_max=100, alpha_factor=0.5, initial_z=None, iteration_number=100, save_name='None', save=False, early_stop=False):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        alpha=alpha_max
    ...
    def optim_z_sparse_deviations(self,alpha=0.1, beta=0.8, initial_z=None, initial_u=None, iteration_number=50 ):
        ...
        return(proj_z(final_result), proj_x(final_result))
         ...
         # ||A(G(z)+u)-y||_2^2+\lambda\mu||z||^2_2+\lamba||u||_1
    def optim_z_sparse_regularisation_parameter(self, lambda_min=0.2, lambda_max=100, lambda_factor=0.5, mu_min=0.01, mu_max=10, mu_factor=0.25,initial_z=None, initial_u=None, iteration_number=100, save_name='None', early_stop=True, save=False):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        if initial_u is None:
            initial_u = np.zeros((1, *self.generative_model.image_shape))
    ...
    def optim_x_soft_constraints_regularisation_parameter(self, lambda_min=0.01, lambda_max=100, lambda_factor=0.5, mu_min=0.2, mu_max=10, mu_factor=0.25,initial_z=None, initial_x=None, iteration_number=100, save_name='None', early_stop=True, save=False):
        if initial_z is None:
            initial_z = np.random.normal(0,1,(1,self.n_latent))
        if initial_x is None:
            initial_x = np.random.normal(0,1,(1,*self.generative_model.image_shape))  
    ...
    def optim_x_tik_regularisation_parameter(self, beta_min=0.2, beta_max=100, beta_factor=0.5, initial_x=None, iteration_number=100, save_name='None', save=False, early_stop=True):
        if initial_x is None:
            initial_x = np.random.normal(0,1,(1,*self.generative_model.image_shape))
        if len(initial_x.shape)==2:
            initial_x=np.expand_dims(initial_x, axis=0)
