...
test_images=np.load('../../knees/datasets/knee_fastMRI_test_128_cleaned.npy')
train_images=np.load('../../knees/datasets/knee_fastMRI_train_128_cleaned.npy')
...
np.save('data_knees_tomo_test_cleaned_253_0.02_noise.npy', inv_prob.data)
...
plot=opt.gd_z_regularisation_parameter(initial_z=np.random.normal(0,1,(4,opt.n_latent)), iteration_number=400, save_name='VAE2_128_128_800_0.01_sig_z_optimisation_knees_cleaned_more_253', save =True)#, alpha_min=0.5, alpha_max=0.6)
plot=opt.optim_z_sparse_regularisation_parameter(initial_z=np.random.normal(0,1,(4,opt.n_latent)), initial_u=np.zeros((4,128,128)), iteration_number=400,  save_name='VAE2_128_128_800_0.01_sig_z_sparse_knees_cleaned_more_253_', early_stop=False, save=True)#, lambda_min=0.5, lambda_max=0.6, mu_min=0.5, mu_max=0.6)
plot=opt.optim_x_soft_constraints_regularisation_parameter(initial_z=np.random.normal(0,1,(4,opt.n_latent)), initial_x=np.random.normal(0,1,(4,128,128)), iteration_number=400, save_name='VAE2_128_128_800_0.01_sig_x_soft_knees_cleaned_more_253_', early_stop=False, save=True)#, lambda_min=0.5, lambda_max=0.6, mu_min=0.5, mu_max=0.6 )
plot=opt.optim_x_tik_regularisation_parameter( iteration_number=6000, save_name='x_tik_knees_test_cleaned_more_253', save=True, early_stop=False) #, beta_min=0.5, beta_max=0.6)
