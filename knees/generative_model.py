...
class generative_model(object):
        ...
        for j in range(initial_z.shape[0]):
            ...
            for i in range(100000):
                ...
                if np.all(self.observationLoss(z_1,y)<= \
                              ...
                              -(1/(2*100*L))*np.linalg.norm(self.gradients(z_0,y), ord=2)**2): #debug +1 set to -1 29-04-2021 
                    z_0=z_1
                    count+=1
                    L=lower*L
                else:
    ...
    def projectManifold(self,x, saveName='test',restore=True, noise_level=0.1, random_seed=99):
        print(np.shape(x))
