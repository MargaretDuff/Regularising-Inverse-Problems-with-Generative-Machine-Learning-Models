...
model=vae.kneesVAE(800,'./checkpoints_no_sigmoid/checkpoints2_128_cleaned_800_kl_factor_0.001/knees_VAE-249800',sess )
...
np.save('aim_knees_test_cleaned_497.npy', inv_prob.aim)
np.save('data_knees_tomography_test_cleaned_497_0.02_noise.npy', inv_prob.data)
...
plot=opt.gd_z_regularisation_parameter(initial_z=np.random.normal(0,1,(4,opt.n_latent)), iteration_number=20, save_name='VAE2_128_128_800_0.001_z_optimisation_knees_test_cleaned_497', save =True)
plot=opt.optim_z_sparse_regularisation_parameter(initial_z=np.random.normal(0,1,(4,opt.n_latent)), initial_u=np.zeros((4,128,128)), iteration_number=20,  save_name='VAE2_128_128_800_0.001_z_sparse_knees_test_cleaned_497_', early_stop=False, save=True)
plot=opt.optim_x_soft_constraints_regularisation_parameter(initial_z=np.random.normal(0,1,(4,opt.n_latent)), initial_x=np.random.normal(0,1,(4,128,128)), iteration_number=50, save_name='VAE2_128_128_800_0.001_x_soft_knees_test_cleaned_497_', early_stop=False, save=True)
plot=opt.optim_x_tik_regularisation_parameter( iteration_number=1500, save_name='x_tik_knees_test_cleaned_497', save=True, early_stop=False)
