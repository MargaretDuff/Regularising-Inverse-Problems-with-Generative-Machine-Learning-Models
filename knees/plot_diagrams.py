...
plt.subplot2grid((3,4), (0,0), colspan=2, rowspan=2)
plot=ae_x_soft
plt.scatter(plot[:,0], plot[:,2], c=plot[:,1],s=5)
plt.colorbar()
plt.hlines(28*28*0.1**2, np.min(plot[:,0]), np.max(plot[:,0]))
plt.xscale('log')
plt.xlabel('lambda')
...
plot=ae_z_opt
axes[0,0].scatter(plot[:,0], plot[:,1], s=5)
...
plot=ae_x_soft
im=axes[0,1].scatter(plot[:,0], plot[:,2], c=plot[:,1],s=5)
...
plot=ae_z_sparse
im=axes[0,2].scatter(plot[:,0], plot[:,2], c=plot[:,1], s=5)
...
plot=ae_x_tik
axes[0,3].scatter(plot[:,0], plot[:,1], s=5)
...
plt.xticks([])
plt.colorbar(im)
plt.xlabel('Observed Data', fontsize=26)

fig.tight_layout()
fig.set_size_inches(w=11,h=6)
fig_name = 'mnist_convolution_153_aim_data.png'
fig.savefig(fig_name)